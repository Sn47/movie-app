...
export function getGenreById(id) {
  return moviesData.genres.find((genre) => genre.id === id)
}
...
export function getDirectorById(id) {
  return moviesData.directors.find((director) => director.id === id)
}

export function getMoviesByGenre(genreId) {
  return moviesData.movies.filter((movie) => movie.genreId === genreId)
}

export function getMoviesByDirector(directorId) {
  return moviesData.movies.filter((movie) => movie.directorId === directorId)
}

export function getTrendingMovies() {
  // For this example, we'll consider movies with rating > 8.5 as trending
  return moviesData.movies.filter((movie) => movie.rating > 8.5)
}
